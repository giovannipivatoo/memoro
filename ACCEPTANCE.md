...
Verifiche del 25 settembre 2026. Risposte, scelta multipla, ripasso libero e pet sviluppati sul branch `feat/answer-modes-practice-pet`; configurazione e prove API reali su `feat/deepseek-live-setup`. Gli oracle Anki esterni sotto riportati appartengono alla v0.3; la v0.4 conserva quel codice e riesegue i test Android di interoperabilità. Android 16/API 36, emulatore arm64 Pixel 7 dedicato; JDK 17. CI aggiunge Android API 35 x86_64.
...
- [x] R04/R06: chiamate a DeepSeek reale con chiave utente, dall'app installata: esempio corretto nelle impostazioni e risposta parziale nello studio con omissione identificata. Dettagli nella sezione API v0.4.
...
- [x] R12: compilazione APK installabile, **9 test unitari e 25 test strumentali**, lint senza errori; documentazione di uso, build e limiti.
...
| `DeepSeekSettingsTest` | Apertura e salvataggio senza invii; prova esplicita e feedback inline; modifiche non salvate disabilitano la prova; cambio modello e errore 422 |
| `DeepSeekStudyTest` | Correzione simulata nel flusso completo: feedback persistito, rettifica separata dall'esito automatico, scadenza invariata fino al voto umano |
...
`testDebugUnitTest lintDebug assembleDebug assembleDebugAndroidTest` completati: 9 test unitari, zero errori lint e 17 warning preesistenti. Suite Android completa: 25/25 su un emulatore dedicato senza credenziali reali, separato dall'emulatore personale. La verifica copre anche chiave con caratteri di controllo senza invio o esposizione del valore, JSON richiesto esplicitamente, thinking disabilitato, errori 400/401/402/422/429/5xx e nessun retry automatico.

Prove reali con `deepseek-flash` e dati sintetici, senza usare i mazzi personali:

- Preflight HTTPS dal Mac: HTTP 200, `finish_reason=stop`, esito `CORRECT` e schema di feedback completo.
- **Prova correzione** nelle impostazioni dell'Activity reale: esempio Roma corretto e [conferma visibile](docs/screenshots/deepseek-settings.png). Chiave inserita tramite il campo protetto, cifrata con Android Keystore e conservata durante l'aggiornamento dell'APK.
- Carta del mazzo «Prova API»: domanda sulle capitali di Italia e Francia, riferimento «Roma e Parigi», risposta «Roma». DeepSeek restituisce [esito parziale, spiegazione e omissione «Parigi»](docs/screenshots/deepseek-partial.png). Ripasso libero completato e presente nella cronologia separata.

Chiave assente da sorgenti, APK e documenti; nessuna credenziale usata nei test automatici o in CI. La prova live conferma i casi sopra, non l'accuratezza del modello su ogni materia: la decisione finale rimane umana. Conflitti fonte e guasti di rete/API sono verificati con risposte simulate.
